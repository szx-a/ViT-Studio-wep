...
| 26 | 合并 | 数据集图片按需加载，避免一次性返回 27,000 个文件名 |
| 27 | 合并 | Chart.js 本地化，解决 CDN 加载失败导致图表空白 |
| 28 | 合并 | 评分百分比进度条 + 加载/推理阶段进度反馈 |
| 29 | 合并 | 评分接口线程化，支持前端实时轮询进度 |
| 30 | 合并 | 训练停止逻辑优化：停止后跳过验证，立即结束 |
| 31 | 合并 | 删除/保留模型后页面状态重置，评分结果内可直接保留/删除 |
...
- **图表可靠**：Chart.js 已本地化到 `server/static/vendor/`，不再依赖 CDN。
- **大数据集性能**：数据集图片列表改为按需请求，避免大响应拖慢页面。
- **评分进度**：评分接口在线程池执行，前端每 0.3 秒轮询百分比。
- **训练停止**：点击停止后跳过验证，快速回到结束状态。
- **状态恢复**：删除或保留训练模型后，训练页自动回到“空闲 / 等待开始训练”状态。
